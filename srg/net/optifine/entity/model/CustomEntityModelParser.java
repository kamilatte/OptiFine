...
public class CustomEntityModelParser {
  ...
  public static CustomModelRenderer parseCustomModelRenderer(JsonObject elem, int[] textureSize, String basePath) {
    ...
    CustomEntityModel customEntityModel = new CustomEntityModel(RenderType::entityCutoutNoCull);
    if (textureSize != null) {
      ((Model)customEntityModel).textureWidth = textureSize[0];
      ((Model)customEntityModel).textureHeight = textureSize[1];
    } 
    ...
    ModelPart mr = PlayerItemParser.parseModelRenderer(elem, (Model)customEntityModel, textureSize, basePath);
