...
public class CustomColormap implements CustomColors.IColorizer {
  ...
  public boolean isValid(String path) {
    if (this.format == 0 || this.format == 1) {
      ...
      if (this.color < 0) {
        ...
        if (this.format == 1)
          this.color = getColorGrid(BiomeUtils.PLAINS, new jd(0, 64, 0)); 
      } 
  ...
  public boolean matchesBlock(dtc blockState) {
  ...
  public int getColor(dtc blockState, dbz blockAccess, jd blockPos) {
    return getColor(blockAccess, blockPos);
  }
  
  public int getColor(dbz blockAccess, jd blockPos) {
    ddw biome = CustomColors.getColorBiome(blockAccess, blockPos);
  ...
  public int getColor(ddw biome, jd blockPos) {
    if (this.format == 0)
  ...
  public int getColorSmooth(dbz blockAccess, double x, double y, double z, int radius) {
    if (this.format == 2)
      return this.color; 
    int x0 = ayo.a(x);
    int y0 = ayo.a(y);
    ...
    for (int ix = x0 - radius; ix <= x0 + radius; ix++) {
      for (int iz = z0 - radius; iz <= z0 + radius; iz++) {
        ...
        int col = getColor(blockAccess, (jd)blockPosM);
        sumRed += col >> 16 & 0xFF;
        sumGreen += col >> 8 & 0xFF;
        sumBlue += col & 0xFF;
        count++;
  ...
  private int getColorVanilla(ddw biome, jd blockPos) {
    double temperature = ayo.a(biome.g(), 0.0F, 1.0F);
    double rainfall = ayo.a(BiomeUtils.getDownfall(biome), 0.0F, 1.0F);
  ...
  private int getColorGrid(ddw biome, jd blockPos) {
