...
public class CustomSkyLayer {
  ...
  public void render(dcw world, fbi matrixStackIn, int timeOfDay, float celestialAngle, float rainStrength, float thunderStrength) {
    ...
    if (this.rotate) {
      ...
      if (this.speed != Math.round(this.speed)) {
        long worldDay = (world.aa() + 18000L) / 24000L;
  ...
  private float getPositionBrightness(dcw world) {
  ...
  private float getPositionBrightnessRaw(dcw world) {
    bsr renderViewEntity = fgo.Q().an();
    if (renderViewEntity == null)
      return 0.0F; 
    jd pos = renderViewEntity.do();
    if (this.biomes != null) {
      ddw biome = (ddw)world.t(pos).a();
      if (biome == null)
        return 0.0F; 
  ...
  private void addVertex(Matrix4f matrix4f, fbd buffer, float x, float y, float z, float u, float v) {
    ...
    buffer.a(xt, yt, zt).a(u, v);
  }
  
  public boolean isActive(dcw world, int timeOfDay) {
