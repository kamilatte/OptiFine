...
public abstract class ShaderOption {
  ...
  public void addPaths(String[] newPaths) {
    ...
    for (int i = 0; i < newPaths.length; i++) {
      ...
      if (!pathList.contains(newPath))
        this.paths = (String[])Config.addObjectToArray((Object[])this.paths, newPath); 
    } 
  }
  ...
  public float getIndexNormalized() {
    ...
    if (index < 0)
      return 0.0F; 
    float f = 1.0F * index / (this.values.length - 1.0F);
    return f;
  }
  ...
  public void setIndexNormalized(float f) {
    ...
    int index = Math.round(f * (this.values.length - 1));
    this.value = this.values[index];
  }
  
  public String toString() {
    return this.name + ", value: " + this.name + ", valueDefault: " + this.value + ", paths: " + this.valueDefault;
  }
}
