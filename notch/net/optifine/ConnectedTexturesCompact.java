...
public class ConnectedTexturesCompact {
  ...
  public static gfw[] getConnectedTextureCtmCompact(int ctmIndex, ConnectedProperties cp, int side, gfw quad, RenderEnv renderEnv) {
  ...
  private static gfw[] getQuadsCompactH(int indexLeft, int indexRight, gql[] sprites, int side, gfw quad, RenderEnv renderEnv) {
    return getQuadsCompact(Dir.LEFT, indexLeft, Dir.RIGHT, indexRight, sprites, side, quad, renderEnv);
  }
  
  private static gfw[] getQuadsCompactV(int indexUp, int indexDown, gql[] sprites, int side, gfw quad, RenderEnv renderEnv) {
    return getQuadsCompact(Dir.UP, indexUp, Dir.DOWN, indexDown, sprites, side, quad, renderEnv);
  }
  
  private static gfw[] getQuadsCompact4(int upLeft, int upRight, int downLeft, int downRight, gql[] sprites, int side, gfw quad, RenderEnv renderEnv) {
    if (upLeft == upRight) {
  ...
  private static gfw[] getQuadsCompact(int index, gql[] sprites, gfw quad, RenderEnv renderEnv) {
    gql sprite = sprites[index];
    return ConnectedTextures.getQuads(sprite, quad, renderEnv);
  }
  
  private static gfw[] getQuadsCompact(Dir dir1, int index1, Dir dir2, int index2, gql[] sprites, int side, gfw quad, RenderEnv renderEnv) {
    gfw quad1 = getQuadCompact(sprites[index1], dir1, side, quad, renderEnv);
    gfw quad2 = getQuadCompact(sprites[index2], dir2, side, quad, renderEnv);
    return renderEnv.getArrayQuadsCtm(quad1, quad2);
  }
  
  private static gfw[] getQuadsCompact(Dir dir1, int index1, Dir dir2, int index2, Dir dir3, int index3, gql[] sprites, int side, gfw quad, RenderEnv renderEnv) {
    gfw quad1 = getQuadCompact(sprites[index1], dir1, side, quad, renderEnv);
    gfw quad2 = getQuadCompact(sprites[index2], dir2, side, quad, renderEnv);
    gfw quad3 = getQuadCompact(sprites[index3], dir3, side, quad, renderEnv);
    return renderEnv.getArrayQuadsCtm(quad1, quad2, quad3);
  }
  
  private static gfw[] getQuadsCompact(Dir dir1, int index1, Dir dir2, int index2, Dir dir3, int index3, Dir dir4, int index4, gql[] sprites, int side, gfw quad, RenderEnv renderEnv) {
    gfw quad1 = getQuadCompact(sprites[index1], dir1, side, quad, renderEnv);
    gfw quad2 = getQuadCompact(sprites[index2], dir2, side, quad, renderEnv);
    gfw quad3 = getQuadCompact(sprites[index3], dir3, side, quad, renderEnv);
    gfw quad4 = getQuadCompact(sprites[index4], dir4, side, quad, renderEnv);
    return renderEnv.getArrayQuadsCtm(quad1, quad2, quad3, quad4);
  }
  
  private static gfw getQuadCompact(gql sprite, Dir dir, int side, gfw quad, RenderEnv renderEnv) {
    switch (dir.ordinal()) {
      case 0:
  ...
  private static gfw getQuadCompact(gql sprite, Dir dir, int x1, int y1, int x2, int y2, int side, gfw quadIn, RenderEnv renderEnv) {
    ...
    Map<gfw, gfw> quadMap = quadMaps[dir.ordinal()];
    if (quadMap == null) {
      quadMap = new IdentityHashMap<>(1);
      quadMaps[dir.ordinal()] = quadMap;
    } 
    gfw quad = quadMap.get(quadIn);
    if (quad == null) {
  ...
  private static gfw makeSpriteQuadCompact(gfw quad, gql sprite, int side, int x1, int y1, int x2, int y2) {
  ...
  private static void fixVertexCompact(int[] data, int vertex, gql spriteFrom, gql spriteTo, int side, int x1, int y1, int x2, int y2) {
    float cu, cv;
    ...
    float atlasW = spriteFrom.getWidth() / (spriteFrom.d() - spriteFrom.c());
    float atlasH = spriteFrom.getHeight() / (spriteFrom.h() - spriteFrom.g());
