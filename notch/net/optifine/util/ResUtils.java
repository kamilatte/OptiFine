...
public class ResUtils {
  ...
  public static String[] collectFiles(asq rp, String prefix, String suffix, String[] defaultPaths) {
    return collectFiles(rp, new String[] { prefix }, new String[] { suffix }, defaultPaths);
  }
  
  public static String[] collectFiles(asq rp, String[] prefixes, String[] suffixes) {
    return collectFiles(rp, prefixes, suffixes, null);
  }
  
  public static String[] collectFiles(asq rp, String[] prefixes, String[] suffixes, String[] defaultPaths) {
    if (rp instanceof asj) {
      asj crp = (asj)rp;
    ...
    if (rp instanceof asu)
      return collectFilesFixed(rp, defaultPaths); 
    File tpFile = null;
    if (rp instanceof asn) {
      asn fpr = (asn)rp;
      tpFile = fpr.getFile();
    } else if (rp instanceof ast) {
      ast ppr = (ast)rp;
      tpFile = ppr.e.toFile();
    } else {
  ...
  private static String[] collectFilesFixed(asq rp, String[] paths) {
    if (paths == null)
      return new String[0]; 
