...
public class CustomSky {
      ...
      for (int j = 0; j < 1000; j++) {
        ...
        try {
          ...
          if (sl.isValid(path)) {
            ...
            akr locSource = new akr(srcPath);
            gpw tex = TextureUtils.getTexture(locSource);
            if (tex == null) {
  ...
  public static void renderSky(dcw world, fbi matrixStackIn, float partialTicks) {
  ...
  public static boolean hasSkyLayers(dcw world) {
    if (worldSkyLayers == null)
      return false; 
    int dimId = WorldUtils.getDimensionId(world);
    if (dimId < 0 || dimId >= worldSkyLayers.length)
