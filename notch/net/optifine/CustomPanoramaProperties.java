...
public class CustomPanoramaProperties {
  ...
  public CustomPanoramaProperties(String path, Properties props) {
    ...
    this.path = path;
    this.panoramaLocations = new akr[6];
    for (int i = 0; i < this.panoramaLocations.length; i++)
      this.panoramaLocations[i] = new akr(path + "/panorama_" + path + ".png"); 
    ...
    this.overlay2Bottom = ConnectedParser.parseColor4(props.getProperty("overlay2.bottom"), -2147483648);
  }
  
  public akr[] getPanoramaLocations() {
  ...
  public String toString() {
    return this.path + ", weight: " + this.path + ", blur: " + this.weight + " " + this.blur1 + " " + this.blur2 + ", overlay: " + this.blur3 + " " + this.overlay1Top + " " + this.overlay1Bottom + " " + this.overlay2Top;
  }
}
